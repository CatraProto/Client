...
namespace CatraProto.Client.Async.Collections
{
    public class BrowsableQueue<T> : IEnumerable<BrowsableQueue<T>.QueuedItemHandle>
    {
        ...
        public bool TryPeek([MaybeNullWhen(false)] out QueuedItemHandle queuedItemHandle)
        {
            lock (_mutex)
            {
                if (_list.Count - 1 >= _iteratorPosition)
                {
                    queuedItemHandle = _lastHandle = new QueuedItemHandle(_list[_iteratorPosition++], this);
                    return true;
                }
        ...
        private void RemoveFromHandle(QueuedItemHandle queuedItemHandle)
        {
            lock (_mutex)
            {
        ...
        public class BrowsableEnumerator : IEnumerator<QueuedItemHandle>
        {
            ...
            {
                return _browsableQueue.TryPeek(out _queuedItemHandle);
            }
        ...
        public class QueuedItemHandle
        {
            ...
            internal QueuedItemHandle(T item, BrowsableQueue<T> browsableQueue)
            {
                _browsableQueue = browsableQueue;
                Item = item;
            }
            ...
            {
                _browsableQueue.RemoveFromHandle(this);
            }
        }
    }
}