...
namespace CatraProto.Client.TL.Requests.CloudChats
{
	public partial class Upload
	{
		...
		public async Task<RpcMessage<FileBase>> GetFileAsync(InputFileLocationBase location, int offset, int limit, bool precise = true,
			bool cdnSupported = true, CancellationToken cancellationToken = default)
		{
			if (location is null)
			{
		...
		public async Task<RpcMessage<FileHashBase>> GetCdnFileHashesAsync(byte[] fileToken, int offset, CancellationToken cancellationToken = default)
		{
			if (fileToken is null)
			{
				throw new ArgumentNullException(nameof(fileToken));
