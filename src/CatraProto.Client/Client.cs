	...
	public class Client
	{
		...
		public async Task Test()
		{
			...
			if (!reqPq.RpcCallFailed)
			{
				...
				var reqDh = await Api.MtProtoApi.ReqDHParamsAsync(nonce, serverNonce, decomposedPq.Item1, decomposedPq.Item2, foundKey,
					encryptedData);
				if (!reqDh.RpcCallFailed)
				{
					if (reqDh.Response is ServerDHParamsOk ok)
					{
						...
						_logger.Information("Message decrypted, checking nonce ({SSNonce} == {Nonce}) and serverNonce ({SSSNonce} == {SNonce})",
						...
						if (!setDhClient.RpcCallFailed)
						{
							...
							{
								_logger.Information("Received dhGenOk checking ({SSNonce} == {Nonce}) and serverNonce ({SSSNonce} == {SNonce})",
