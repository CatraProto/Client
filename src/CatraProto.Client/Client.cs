    ...
    public class Client
    {
        ...
        public async Task Test()
        {
            ...
            if (!reqPq.RpcCallFailed)
            {
                ...
                _logger.Information("Received ServerNonce from server with value {SNonce} and Pq with value {Pq}",
                    serverNonce, pq);
                ...
                var encryptedData =
                    rsaKey.EncryptData(Hashing.ComputeDataHashedFilling(toHash, MergedProvider.Singleton));
                _logger.Information("Sending ReqDHParams request...");
                var reqDh = await Api.MtProtoApi.ReqDHParamsAsync(nonce, serverNonce, decomposedPq.Item1,
                    decomposedPq.Item2, foundKey,
                    encryptedData);
                if (!reqDh.RpcCallFailed)
                {
                    if (reqDh.Response is ServerDHParamsOk ok)
                    {
                        ...
                        _logger.Information(
                            "Message decrypted, checking nonce ({SSNonce} == {Nonce}) and serverNonce ({SSSNonce} == {SNonce})",
                        ...
                        var hashedPadding =
                            Hashing.ComputeDataHashedPadding(clientDhInnerData, MergedProvider.Singleton);
                        ...
                        if (!setDhClient.RpcCallFailed)
                        {
                            ...
                            {
                                _logger.Information(
                                    "Received dhGenOk checking ({SSNonce} == {Nonce}) and serverNonce ({SSSNonce} == {SNonce})",
                                    dhGenOk.Nonce, nonce, dhGenOk.ServerNonce, serverNonce);
                                KeyExchangeChecks.CheckNonce(dhGenOk.Nonce, nonce);
                                KeyExchangeChecks.CheckNonce(dhGenOk.ServerNonce, serverNonce);

                                var authKey = BigInteger.ModPow(new BigInteger(serverDhInnerData.GA), b, dhPrime)
                                    .ToByteArray();
                                var authKeyId = SHA1.HashData(authKey).TakeLast(8).ToArray();
                                _logger.Information(
                                    "Nonce and serverNonce match, AuthKey successfully generated, AuthKeyId: {Id}",
                                    authKeyId);
                            }
                        }
                    }
