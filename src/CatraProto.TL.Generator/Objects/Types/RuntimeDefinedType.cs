...
namespace CatraProto.TL.Generator.Objects.Types
{
    public class RuntimeDefinedType : TypeBase
    {
        ...
        public override void WriteDeserializer(StringBuilder stringBuilder, Parameter parameter)
        {
            ...
            stringBuilder.AppendLine(parameter.VectorInfo.IsVector ? $"{spacing}{parameter.NamingInfo.PascalCaseName} = reader.ReadVector<{GetTypeName(NamingType.FullNamespace, parameter, false)}>();" : $"{spacing}{parameter.NamingInfo.PascalCaseName} = reader.Read<{GetTypeName(NamingType.FullNamespace, parameter, false)}>();");
