...
namespace CatraProto.TL.Generator.Objects
{
	class Method : Object
	{
		...
		public override void WriteParameters(StringBuilder builder)
		{
			...
			builder.AppendLine($"{StringTools.TwoTabs}public System.Type " + (Parameters.Exists(x => x.Name == "Type") ? "IMethod." : "") +
			                   $"Type {{ get; init; }} = typeof({type});");
			builder.AppendLine($"{StringTools.TwoTabs}public bool " + (Parameters.Exists(x => x.Name == "IsVector") ? "IMethod." : "") +
			                   $"IsVector {{ get; init; }} = {ReturnsVector.ToString().ToLower()};");
		...
		public void WriteMethod(StringBuilder builder)
		{
			...
			builder.AppendLine(
				$"{StringTools.TwoTabs}public async Task<RpcMessage<{returnType}>> {Name}Async({args}{comma} CancellationToken cancellationToken = default)\n{StringTools.TwoTabs}{{");
